...
def dashboard_view(request):
    ...
    context = {
        ...
        'user_photo_url': account.photo_profile.url if account.photo_profile else '/tabletap/static/user.png',
...
def remove_staff_ajax_view(request):
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        ...
        # return the updated staff & invites for that restaurant
        try:
            resto = Restaurant.objects.get(res_id=restaurant_id)
        except Restaurant.DoesNotExist:
...
def manage_orders_view(request):
    ...
    # show the restaurant where user is owner or staff
    user_restaurants = Restaurant.objects.filter(
        Q(user=account) | Q(staff_members__user=account)
    ).distinct().order_by('name')
    ...
    # when user chose any restaurant (related to filter)
...
# ---------- AJAX for manage_orders (no full page reload) ----------
@csrf_exempt
...
def manage_tables_view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'create_table':
            ...
            qr_url = request.build_absolute_uri(f"/tabletap/application/menu/{new_table.table_id}/")
...
def regenerate_qr_view(request, table_id):
    ...
    # Generate new QR
    qr_url = request.build_absolute_uri(f"/tabletap/application/menu/{table_id}/")
    qr_img = qrcode.make(qr_url)
    buffer = io.BytesIO()
    qr_img.save(buffer, format='PNG')
...
def make_order_view(request):
    # Try to load user from JWT, but allow anonymous if missing/invalid
    account = None
    token = request.session.get('jwt_token')
    if token:
        try:
    ...
    table_id = request.session.get('table_id')
    if table_id:
        table_obj = RestaurantTable.objects.filter(table_id=table_id).first()

    # Determine branch
    if table_obj:
        branch = table_obj.branch
    else:
        # fallback for anonymous: pick first branch of first restaurant
        resto = Restaurant.objects.filter(user=account).first() if account else None
    ...
    # Populate OrderItems and compute total
    order_total = Decimal('0.00')
    for menu_id_str, qty in cart.items():
        menu_item = MenuItem.objects.filter(menu_id=int(menu_id_str)).first()
        if not menu_item:
            continue
        subtotal = menu_item.price * qty
